...
def threshold_fit(X, y, alpha, n_class, mode='AE',
    ...
    sol = optimize.minimize(obj_margin, x0, method='L-BFGS-B',
        ...
        args=(X, y.astype(int), alpha, n_class, loss_fd, L, sample_weight),
        tol=tol)
...
class LogisticAT(base.BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self.coef_, self.theta_ = threshold_fit(
            X, y_tmp, self.alpha, int(self.n_class_), mode='AE',
    ...
    def score(self, X, y, sample_weight=None):
        pred = self.predict(X)
        return -1 * metrics.mean_absolute_error(pred, y, sample_weight=sample_weight)
...
class LogisticIT(base.BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        y_tmp = y - y.min()  # we need classes that start at zero
        self.coef_, self.theta_ = threshold_fit(
            X, y_tmp, self.alpha, int(self.n_class_),
...
class LogisticSE(base.BaseEstimator):
    ...
    def score(self, X, y, sample_weight=None):
        pred = self.predict(X)
        return -1*metrics.mean_squared_error(
            pred,
            y,
            sample_weight=sample_weight)
